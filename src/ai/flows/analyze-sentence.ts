...
Para el campo "sentenceParts": Analiza la oración "{{{sentence}}}" y divídela en sus componentes individuales (palabras y signos de puntuación). Para cada componente, proporciona el texto y un 'role' simplificado (ej. "verb", "noun", "adjective", "adverb", "pronoun", "preposition", "conjunction", "determiner", "interjection", "contraction", "punctuation", "other"). Es CRUCIAL que este 'role' simplificado refleje con precisión la función gramatical principal de la palabra en el contexto de la oración proporcionada, ya que este 'role' se utiliza directamente para aplicar colores en la interfaz de usuario. Las contracciones comunes como "I'm", "don't", "it's", "you're", "she's", "he's", "we're", "they're", "isn't", "aren't", "wasn't", "weren't", "hasn't", "haven't", "hadn't", "doesn't", "didn't", "won't", "wouldn't", "shan't", "shouldn't", "can't", "couldn't", "mustn't" deben identificarse como una sola unidad con el rol "contraction". También incluye un 'rawRole' con la descripción gramatical más detallada en español (ej. "Verbo (Transitivo)", "Sustantivo (Común)", "Contracción"). Asegúrate de que la secuencia de sentenceParts reconstruya la oración original exactamente, incluyendo espacios y puntuación como partes separadas si es necesario para la reconstrucción fiel. Por ejemplo, "Hello, world!" podría ser [{text: "Hello", role: "interjection"}, {text: ",", role: "punctuation"}, {text: " ", role: "space"}, {text: "world", role: "noun"}, {text: "!", role: "punctuation"}]. Sin embargo, es preferible que las palabras mantengan su integridad y los espacios se manejen en la UI. Así que "Hello, world!" podría ser [{text: "Hello", role: "interjection"}, {text: ",", role: "punctuation"}, {text: "world", role: "noun"}, {text: "!", role: "punctuation"}]. Los espacios entre palabras son implícitos.
...
export type { ImproveSentenceInput, ImproveSentenceOutput } from '@/ai/flows/improve-sentence';
 ...
 // Make sure ExtendedAnalyzeSentenceOutput is not re-exported here if it's primarily defined in lib/types.ts
 // export type { AnalyzeSentenceOutput as ExtendedAnalyzeSentenceOutput } from './analyze-sentence';
 // Schemas are not exported from "use server" files.
 // export { ImproveSentenceInputSchema, ImproveSentenceOutputSchema } from '@/ai/flows/improve-sentence';
